...
from ODIR_DATASET import ODIR_DATASET
...
parser.add_argument('--ratio', type=float, nargs='+', default=[0.8, 1], metavar='RATIO',
...
def main(fold_i = 0, data_ = None, train_index = None, val_index = None):
    ...
    writer=None
    if args.rank==0:
        writer=SummaryWriter(f'{args.output}/fold{fold_i}')
    ...
    dataset_eval=ODIR_DATASET(images_names=val_data,transform=valid_trans)   
...
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.distributed:
            if all_target is None:
                all_target = gather_tensor(target)
                all_output = gather_tensor(output)
            else:
                all_target = torch.cat(
                    (all_target, gather_tensor(target)), dim=0)
                all_output = torch.cat(
                    (all_output, gather_tensor(output)), dim=0)
        num_updates += 1
