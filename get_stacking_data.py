...
from ODIR_DATASET import ODIR_TEST_DATASET,TEST_DATA_PATH,TRAIN_DATA_PATH,ODIR_VALID_DATASET,VALID_DATA_PATH,TEST_DATA_PATH,ODIR_DATASET,ODIR_DATASET_TRAIN_FOR_STACKING
import os
...
def validate(model, loader): 
    ...
    with torch.no_grad():
        for batch_idx, (input, target,name) in pbar:
            input = input.cuda()
            output = model(input)
            preds.append(np.concatenate((output.sigmoid().to('cpu').numpy(),target),axis=1))
...
if __name__ == '__main__':
    ...
    test_index = ['ID','N','D','G','C','A','H','M','O','Np','Dp','Gp','Cp','Ap','Hp','Mp','Op']
    ...
    tst_preds = []
    model_path = [
        # '/disk1/wangjialei/research/odir_main/all_fusionloss/train/20230702-100809-res2net50d-384/checkpoint-23.pth.tar',
                ...
                #   '/disk1/wangjialei/research/odir_main/all_fusionloss/train/20230702-104709-resnet50d-384/checkpoint-24.pth.tar',
                  '/disk1/wangjialei/research/odir_main/all_bce_with_weight/train/20230701-213304-res2net50d-384/checkpoint-28.pth.tar'
                  ]
    model_names=[
        # 'res2net50d',
        # 'davit_base',
    ...
    test.to_csv('./stacking/test4.csv', index=False)
